...
public class memoryhierarchy {
    ...
    public static void main(String[] args){
        ...
        List<List<String>> tmp = csvReader.readCSV("D:\\ITM-work\\Computer-Systems\\MemoryHierarchy\\src\\scriptmemory.csv");
        ...
        for(int i=0; i<tmp.size(); i++) {
            ...
            wordData.get(i).index = i;
            wordData.get(i).word = tmp.get(i).get(0);
        }

        //Hit Miss 기록
        int L1_HitMissCount[] = {0, 0};
        int L2_HitMissCount[] = {0, 0};
        int L3_HitMissCount[] = {0, 0};
        int L4_HitMissCount[] = {0, 0};
        ...
        //호출할 데이터 리스트 생성
        List<List<String>> call = csvReader.readCSV("D:\\ITM-work\\Computer-Systems\\MemoryHierarchy\\src\\call.csv");
        ArrayList<Integer> script = new ArrayList<>();

        // 메모리 주소 호출하는 script 어레이 리스트
        for(int i=0; i<call.size(); i++) {
            script.add(Integer.parseInt(call.get(i).get(1)));
        }
...
//          호출하려는 데이터 할당
            Data data  = new Data();
            data.index = script.get(i);
            data.word = call.get(i).get(0);
...
//                hitLayer = 1;
            ...
            if (flg == 0){
                ...
                //size가 0이 아닐 경우 캐시 탐색
                for(int j = 0; j<L2.cacheSize; j++){

                    int L3setidx = (L2.cache.get(j).set[0].tag*16) + L2.cache.get(j).setIndex;
                    ...
                    if( (((L4.cache.get(j).set[0].tag*4096)+ L4.cache.get(j).setIndex) == data.index) && (L4.cache.get(j).set[0].valid == 1) ){ // 수정 필요
                        ...
                        if(L3.cache2way.get(L4setIndex%256).set.length < 2){
                            ...
                            if(L3.cache2way.get(L4setIndex%256).set[0].accessTime > L3.cache2way.get(L4setIndex%256).set[1].accessTime){
                                ...
                                L3.cache2way.get(L4setIndex%256).set[1].accessTime = 0;
                            }else{
                                ...
                                L3.cache2way.get(L4setIndex%256).set[0].accessTime = 0;
                            }
                        }
...
//            System.out.println(L1_HitMissCount[0] + " " + L2_HitMissCount[0] + " " + L3_HitMissCount[0] + " " + L4_HitMissCount[0]);
        }

        float L1_hitRatio = L1_HitMissCount[0]/(float)35885;
        float L2_hitRatio = L2_HitMissCount[0]/(float)35885;
        float L3_hitRatio = L3_HitMissCount[0]/(float)35885;
        float L4_hitRatio = L4_HitMissCount[0]/(float)35885;
